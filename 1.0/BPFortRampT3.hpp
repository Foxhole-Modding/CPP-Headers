#ifndef UE4SS_SDK_BPFortRampT3_HPP
#define UE4SS_SDK_BPFortRampT3_HPP

class ABPFortRampT3_C : public AConcreteFort
{
    class UTemplateComponent* FortCommonDirtT3;
    class UTemplateComponent* FortRampCommonT2T3Mods;
    class UTemplateComponent* FortRampCommonDefenseMods;
    class UTemplateComponent* FortRampCommon;
    class UTemplateComponent* FortRampCommonMods;
    class UTemplateComponent* FortCommon;
    class UStaticMeshComponent* RampFront;
    class UStaticMeshComponent* Roof;
    class UStaticMeshComponent* CornerFrontRight;
    class UStaticMeshComponent* CornerFrontLeft;
    class UStaticMeshComponent* CornerBackLeft;
    class UStaticMeshComponent* Floor;
    class UStaticMeshComponent* CornerBackRight;

};

#endif
